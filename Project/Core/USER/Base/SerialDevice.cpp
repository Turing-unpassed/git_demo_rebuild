...
void SerialDevice::SetRxPackage_identity(Package PKD)
{
    rx_frame_mat.Head_0 = PKD.Head_0;
    rx_frame_mat.Head_1 = PKD.Head_1;
    rx_frame_mat.End_0  = PKD.End_0;
    rx_frame_mat.End_1  = PKD.End_1;
    rx_frame_mat.frame_id = PKD.frame_id;
    rx_frame_mat.length = PKD.length;
		RxPK_ok_flag = 1;
}
...
extern "C" void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  uint8_t rxByte  ;
	for(int i = 0;i<SerialDevice::instanceCount_;i++)
	{
       ...
       HAL_UART_Receive_IT(SerialDevice::instances_[i]->huart_, SerialDevice::instances_[i]->rxBuffer_, RX_BUFFER_SIZE);
		}
	}
    // // 获取 UART 接收的字节
    // for (int i = 0; i < SerialDevice::instanceCount_; i++)
