...
CanDevice::CanDevice(CAN_HandleTypeDef *hcan_, uint8_t can_id_):hcan(hcan_), can_id(can_id_){
    if(hcan_ == &hcan1){
        if(Can1_Instances_Index < MAX_INSTANCES){       //将can1上的所有实例指针存入数组
            Can1_Instances[Can1_Instances_Index] = this;
            Can1_Instances_Index++;
        }
    }else if(hcan_ == &hcan2){
        if(Can2_Instances_Index < MAX_INSTANCES){       //将can2上的所有实例指针存入数组
...
extern "C" void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan){
    if(hcan == &hcan1){
        ...
        for(uint8_t i = 0; i < Can1_Instances_Index; i++){
            Can1_Instances[i]->Can_update(RxData1);        //调用所有实例中的Can_update函数，如何识别是否是该实例所需的报文，由子类实现
        }
    }else if(hcan == &hcan2){
        ...
        for(uint8_t i = 0; i < Can2_Instances_Index; i++){
            Can2_Instances[i]->Can_update(RxData2);
        }
    }
}