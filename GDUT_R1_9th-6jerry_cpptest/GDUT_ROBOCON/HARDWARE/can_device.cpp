...
void CanDevice::registerInstance(CAN_HandleTypeDef *hcan, CanDeviceType deviceType, uint8_t can_id, CanDevice **instances, int &instanceCount) {
    if (instanceCount < MAX_INSTANCES) {
        int insertPos = instanceCount;
        for (int i = 0; i < instanceCount; ++i) {
            if (instances[i]->can_id > can_id) {
                insertPos = i;
                break;
            }
        }

        for (int i = instanceCount; i > insertPos; --i) {
            instances[i] = instances[i - 1];
        }

        instances[insertPos] = this;
        instanceCount++;
    }
}
...
CanDevice::CanDevice(CanDeviceType deviceType_, CAN_HandleTypeDef *hcan_, uint8_t can_id) : deviceType_(deviceType_), hcan_(hcan_), can_id(can_id)
{
    if (hcan_ == &hcan1) {
        ...
        case CanDeviceType::M3508:
            registerInstance(hcan_, deviceType_, can_id, m3508_instances_can1, instanceCount_m3508_can1);
            break;
        case CanDeviceType::M6020:
            registerInstance(hcan_, deviceType_, can_id, m6020_instances_can1, instanceCount_m6020_can1);
            break;
        default:
            break;
        }
        ...
        switch (deviceType_) {
        case CanDeviceType::M3508:
            registerInstance(hcan_, deviceType_, can_id, m3508_instances_can2, instanceCount_m3508_can2);
            break;
        case CanDeviceType::M6020:
            registerInstance(hcan_, deviceType_, can_id, m6020_instances_can2, instanceCount_m6020_can2);
            break;
        default:
            break;
        }
